...
public class CrystalRangeSeekbar extends View {
    ...
    public CrystalRangeSeekbar setBarHighlightHeight(float height) {
        _barHighlightHeight = height;
        return this;
    }
    ...
    protected float getBarHighlightHeight(){
    ...
    protected float getBarHighlightHeight(final TypedArray typedArray){
        return typedArray.getDimensionPixelSize(R.styleable.CrystalRangeSeekbar_bar_highlight_height, 0);
    }
    ...
    public int getMatchingLeftThumbColor() {
        ...
        int[] colors = {barColorBlend, barHighlightColorBlend};
        return blendColors((1 - percent), colors);
    }

    public int getMatchingRightThumbColor() {
        double percent = (getSelectedMaxValue().floatValue() - minValue) / (maxValue - minValue);
        int barHighlightColorBlend = barHighlightColor;
        int barColorBlend = barColor;
        if (barHighlightColorMode != CrystalSeekbar.ColorMode.SOLID) {
            int[] barHighlightGradientColors = {barHighlightGradientStart, barHighlightGradientEnd};
    ...
    protected boolean isThumbColorMatchBarEnabled(final TypedArray typedArray){
        return typedArray.getBoolean(R.styleable.CrystalRangeSeekbar_thumb_color_match_bar, false);
    }
    ...
    protected void setupLeftThumb(final Canvas canvas, final Paint paint, final RectF rect){
        if (thumbColorMatchBarEnabled) {
            leftThumbColor = getMatchingLeftThumbColor();
        } else {
    ...
    protected void setupRightThumb(final Canvas canvas, final Paint paint, final RectF rect){
        if (thumbColorMatchBarEnabled) {
            rightThumbColor = getMatchingRightThumbColor();
        } else {
                ...
                if (pressedThumb != null) {
                    ...
                    if (thumbColorMatchBarEnabled) {
                        if (pressedThumb == Thumb.MIN) {
                            int matchedColor = getMatchingLeftThumbColor();
                            setLeftThumbColor(matchedColor);
                            setLeftThumbHighlightColor(matchedColor);
                        } else {
                            int matchedColor = getMatchingRightThumbColor();
                            setRightThumbColor(matchedColor);
                            setRightThumbHighlightColor(matchedColor);
                        }
                    }
                }
                break;
